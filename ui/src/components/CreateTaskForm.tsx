...
import { APPROVAL_MODES, DEFAULT_APPROVAL_MODE, DEFAULT_ENGINE, ENGINE_OPTIONS } from '../lib/task-utils.ts'

const { Check, Plus, Repeat2, ShieldCheck, Sparkles, X } = lucide

export default function CreateTaskForm({ initialPrompt, initialEngine, initialApprovalMode, initialGoal, onSubmit, onCancel }) {
  const [prompt, setPrompt] = useState(initialPrompt || '')
  const [engine, setEngine] = useState(initialEngine || DEFAULT_ENGINE)
  const [approvalMode, setApprovalMode] = useState(initialApprovalMode || DEFAULT_APPROVAL_MODE)
  ...
  // Closing is UNCONDITIONAL — the close button, Cancel, Escape and a backdrop
  // click all just close. The typed prompt is not lost: the board keeps it as a
  // draft and hands it back the next time this form opens, which is why there is
  // no "discard what you typed?" confirmation to get stuck behind.
  const goalDraft = () => loop ? {
    mode: 'loop',
  ...
  const requestClose = () => onCancel({ prompt, engine, approval_mode: approvalMode, goal: goalDraft() })
  ...
  return _jsx('div', { onClick: event => { if (event.target === event.currentTarget) requestClose() }, style: { position: 'absolute', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: 'rgba(0,0,0,0.35)' }, children: _jsxs('form', { role: 'dialog', 'aria-modal': true, 'aria-label': 'New task', onSubmit: e => { e.preventDefault(); if (prompt.trim()) onSubmit({ prompt: prompt.trim(), engine: loop ? 'task_runner' : engine, approval_mode: approvalMode, goal: goalDraft() }) }, style: { width: '100%', maxWidth: 560, maxHeight: '100%', overflowY: 'auto', background: T.elevated, border: `1px solid ${T.border}`, borderRadius: 12, padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 14 }, children: [
    ...
    _jsx('select', { value: loop ? 'task_runner' : engine, disabled: loop, 'aria-label': 'Engine for this task', onChange: e => setEngine(e.target.value), style: { background: T.bg, color: T.strong, border: `1px solid ${T.border}`, borderRadius: 8, padding: 9, opacity: loop ? 0.72 : 1 }, children: ENGINE_OPTIONS.map(option => _jsx('option', { value: option.id, children: `${option.label} — ${option.help}` }, option.id)) }),
    _jsxs('label', { style: { display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: T.muted }, children: [
      _jsx(ShieldCheck, { size: 13, style: { flexShrink: 0 } }),
      'Permission',
      _jsx('select', { value: approvalMode, 'aria-label': 'Permission for this task', onChange: e => setApprovalMode(e.target.value), style: { flex: 1, minWidth: 0, background: T.bg, color: T.strong, border: `1px solid ${T.border}`, borderRadius: 8, padding: 8, fontSize: 12 }, children: APPROVAL_MODES.map(mode => _jsx('option', { value: mode.id, children: `${mode.label} — ${mode.help}` }, mode.id)) }),
    ] }),
