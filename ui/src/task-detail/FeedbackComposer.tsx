...
// The drawer can now be dismissed by clicking the board, so an unsent
// instruction has to survive the close. It is kept per task and only for this
// browser session: a draft is a half-finished thought, not board state, so it
// belongs in sessionStorage rather than on the server record.
const draftKey = taskId => `kanban:feedback-draft:${taskId}`
const readDraft = taskId => {
  try { return window.sessionStorage.getItem(draftKey(taskId)) || '' } catch { return '' }
}
const writeDraft = (taskId, value) => {
  try {
    if (value) window.sessionStorage.setItem(draftKey(taskId), value)
    else window.sessionStorage.removeItem(draftKey(taskId))
  } catch { /* the draft simply does not survive a close */ }
}

export default function FeedbackComposer({ task, latest, onFeedback, onOpenEngine }) {
  const [feedback, setFeedback] = useState(() => readDraft(task.id))
  ...
  useEffect(() => { setFeedback(readDraft(task.id)); setSent(false) }, [task.id])

  // One writer, so the draft the user sees and the one storage holds cannot drift.
  ...
  return _jsxs('form', { onSubmit: async event => {
    ...
    _jsxs('div', { style: { display: 'flex', gap: 7, padding: 6, borderRadius: 11, background: T.bg, border: `1px solid ${enabled && feedback.trim() ? 'rgba(124,58,237,0.55)' : T.border}` }, children: [
      _jsx('input', { id: `kanban-feedback-${task.id}`, 'aria-label': taskRunner ? 'Give instruction for next goal attempt' : 'Reply to agent', value: feedback, disabled: !enabled, onChange: event => changeFeedback(event.target.value), placeholder, style: { minWidth: 0, flex: 1, padding: '6px 7px', border: 'none', outline: 'none', background: 'transparent', color: T.strong, fontSize: 12, fontFamily: 'inherit', opacity: enabled ? 1 : 0.66 } }),
